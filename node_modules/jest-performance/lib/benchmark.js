 ...
 *
 */
const benchmark = async function (discarded, racersObj) {
  // @ts-ignore
  const suiteName = this.currentTestName; // @ts-ignore

  const testPath = this.testPath;

  const testName = _path.default.basename(testPath);

  const racersPairs = _ramda.default.toPairs(racersObj);

  await new Promise(resolve => {
    const suite = new _benchmark.default.Suite(suiteName);
    racersPairs.reduce((acc, [name, fun]) => {
      // @ts-ignore
      acc.add(name, fun);
      return acc;
